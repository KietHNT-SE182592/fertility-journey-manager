...
import { getServiceClass, getButtonClass, getTextClass } from "@/lib/colors";
    ...
    <section id="services" className="py-20 bg-white">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-16">
          <h2 className={`text-4xl mb-4 ${getTextClass('heading')}`}>
            Our Fertility Services
          </h2>
          <p className={`text-xl max-w-3xl mx-auto ${getTextClass('secondary')}`}>
                  ...
                  <Card className="h-full border-0 shadow-xl bg-gradient-to-br from-white via-blue-50/30 to-teal-50/30 rounded-b-2xl lg:rounded-b-none lg:rounded-r-2xl lg:rounded-l-none">
                    <CardHeader className="pb-4">
                      <div className="flex items-start justify-between mb-4">
                        <div className="space-y-2">
                          <CardTitle className={`text-3xl mb-2 ${getServiceClass('title')}`}>{service.name}</CardTitle>
                          <div className={`text-3xl ${getServiceClass('price')}`}>{service.price}</div>
                        </div>
                      </div>
                      <CardDescription className={`text-lg leading-relaxed ${getServiceClass('description')}`}>
                        {service.description}
                      </CardDescription>
                    </CardHeader>
                    ...
                    <CardContent className="pt-0">
                      <div className="space-y-6">
                        <div>
                          <h4 className={`mb-4 text-lg ${getTextClass('subheading')}`}>What's Included:</h4>
                          <div className="grid grid-cols-1 gap-3">
                            {service.features.map((feature, idx) => (
                              <div key={idx} className="flex items-center">
                                <div className="w-2 h-2 bg-gradient-to-r from-blue-500 to-teal-500 rounded-full mr-4 flex-shrink-0"></div>
                                <span className={`text-base ${getServiceClass('feature')}`}>{feature}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 pt-6">
                          <Button 
                            className={`flex-1 ${getButtonClass('primary')} text-base py-6`}
                          ...
                          <Link to={service.detailLink} className="flex-1">
                            <Button variant="outline" className={`w-full ${getButtonClass('secondary')} text-base py-6`}>
        ...
        <div className="mt-20 text-center bg-gradient-to-r from-blue-50 to-teal-50 rounded-3xl p-12">
          <h3 className={`text-3xl mb-4 ${getTextClass('heading')}`}>
          ...
          <Button 
            size="lg" 
            className={`${getButtonClass('primary')} text-lg px-8`}
